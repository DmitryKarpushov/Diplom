...
public class DataBase {
    ...
    private static final int DEL = 100000000;
    ...
    public void readingBD() throws SQLException {
        ...
        while (result.next()) {
            Customer customer = new Customer(result.getInt("id"), result.getString("name"), result.getDouble("rate"),
                    result.getString("typeloan"), result.getInt("qualitycategory"), result.getDate("dateloanto"),
                    result.getDate("dateloanfrom"), result.getInt("loanamount"));
            customers.add(customer);
            mapCustomers.put(result.getInt("id"), customer);
